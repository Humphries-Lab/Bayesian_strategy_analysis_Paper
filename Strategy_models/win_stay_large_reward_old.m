...
if number_trials > 1 && trial_data.MaxReward(end-1) == "yes"
    if trial_data.Choice(end-1) == trial_data.OptReward(end-1) && trial_data.Choice(end) == trial_data.OptReward(end)       
        % if chose the cued option on previous trial AND chose an
        % cued option on this trial too
        trial_type = "success";        
    elseif trial_data.Choice(end-1) ~= trial_data.OptReward(end-1) && ...
            trial_data.Choice(end) ~= trial_data.OptReward(end)       
