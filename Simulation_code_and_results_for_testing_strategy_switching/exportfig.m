...
%
%   Format Paramter:
%     'Format'  one of the strings 'eps','eps2','jpeg','png','tiff','tiffn','preview'
...
%     'Color'     one of the strings 'bw', 'gray', 'cmyk','rgb'
...
%                 using the CMYK color space (for EPS only)
%         'rgb'   specifies that all objects exported in color using
...
for k = 1:2:length(paramPairs)
  ...
  switch (param)
   case 'format'
    format = value;
    if (~strcmp(format,{'eps','eps2','jpeg','png','preview','tiff','tiffn'}))
      error(['Format must be ''eps'', ''eps2'', ''jpeg'', ''tiff'' , ''tiffn'', ''png'' or' ...
	     ' ''preview''.']);
    end
   case 'preview'
    preview = value;
   ...
   case 'color'
    color = lower(value);
    if (~strcmp(color,{'bw','gray','cmyk','rgb'}))
