%% detection of strategy switching, when strategies are stochastic and change gradually (linear model) and mutually exclusive
% basic model: two strategies are adopted with probability p1 > p2 for t
% trials; then switched to p2 > p1. Probabilities are mutually exclusive,
% so must sum <= 1
%
...
% thin pairs to just those whose sum is <= 1
totals = sum(pairs,2);
pairs(totals > 1,:) = []; % remove them
        ...
        for iSlope = 1:numel(m)
            ...
            for iRepeats = 1:number_of_repeats
                ...
                while p_current >= p_new
                    ...
                    % find new trial outcome using those MUTUALLY EXCLUSIVE probabilities for
                    % each strategy
                    
                    % draw random number and see which strategy it
                    % corresponds to
                    outcome = discreteinvrnd([p1,p2,1-(p1+p2)],1,1); 
                    if outcome == 1 % strategy 1 used
                        x_1_new = 1; x_2_new = 0;
                    elseif outcome == 2 
                        x_1_new = 0; x_2_new = 1;
                    else % neither strategy used
                        x_1_new = 0; x_2_new = 0;
                    end
...
save('Results_for_mutually_exclusive_slope_effects','Results','pairs','p_higher','p_lower','gamma','switching_trial','m')
...
% and how stable is p(new) > p(current)
...
xlabel('slope')
ylabel('stability')
exportPPTfig(gcf,['MutExcl_stability_for_' num2str(pairs(pair_index,1)) '_' num2str(pairs(pair_index,2)) '_crossover.png'],[pwd '\'],[10 15 10 10])
...
% worst-case here
pair_index = find(pairs(:,1) == 0.6 & pairs(:,2) == 0.4);
plot_slope_vs_trials(m,Results,pair_index,pairs)
...
function plot_slope_vs_trials(slope,Results,pair_index,pairs)
 figure
    ...
    exportPPTfig(gcf,['MutExcl_Plateau_slope_for_' num2str(pairs(pair_index,1)) '_' num2str(pairs(pair_index,2)) '_crossover.png'],[pwd '\'],[10 15 20 10])
    ...
    exportPPTfig(gcf,['MutExcl_Plateau_slope_for_' num2str(pairs(pair_index,1)) '_' num2str(pairs(pair_index,2)) 'since_t.png'],[pwd '\'],[10 15 20 10])
